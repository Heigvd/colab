...
const logger = getLogger('LiveChanges.debug');
...
test('DeepBranches: insert and delete', () => {
  ...
  expect(newValue.value).toBe(
    'apricot blueberry cherry durian elderberry\n' + 'asparagus bean cucumber daikon eggplant',
  );
});

test('DeepBranches2: insert and delete', () => {
  const session1 = 's1';
  const session2 = 's2';
  const initialValue = 'apple banana cherry durian\n' + 'asparagus broccoli carrot daikon';
  ...
  const changes: Change[] = [
    ...
    createChange(session1, ['s1-2', 's2-1'], 's1-3', ins(31, ' elderberry')),
    createChange(session2, ['s2-2', 's1-1'], 's2-3', ins(delta + 2 + 30, ' eggplant')),
  ];

  const newValue = LiveHelper.process(initialValue, '0', changes);

  expect(newValue.revision.indexOf('s2-3') >= 0).toBe(true);
  expect(newValue.revision.indexOf('s1-3') >= 0).toBe(true);
