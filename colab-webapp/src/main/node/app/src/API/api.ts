...
export const subscribeToBlockChannel = createAsyncThunk('block/subscribe', async (id: number, thunkApi) => {
  const state = thunkApi.getState() as ColabState;
...
export const unsubscribeFromBlockChannel = createAsyncThunk('block/unsubscribe', async (id: number, thunkApi) => {
  const state = thunkApi.getState() as ColabState;
  const sessionId = state.websockets.sessionId;
